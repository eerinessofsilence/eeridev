    ...
    <footer className="container-small flex items-center justify-between border-t border-border/25 py-6">
        ...
        <nav aria-label="Primary navigation" className="flex flex-wrap gap-6">
          {navigation.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className={`rounded-full transition-colors ${item.href == pathName ? 'hidden' : 'hover:text-text'}`}
            >
              {item.label}
            </a>
          ))}
      ...
      <div className="flex gap-3 text-text-muted/50">
        <a
          ...
          className="hover:text-text-muted/75"
        >
          <Mail className="h-4 w-4" />
        </a>
        <a href="https://x.com/eeridev">
          <svg
            className="h-4 w-4 transition-all duration-200 hover:brightness-300"
            viewBox="0 0 24 24"
            fill="currentColor"
          >
        ...
        <a href="https://github.com/eerinessofsilence">
          <svg
            className="h-4 w-4 transition-all duration-200 hover:brightness-300"
            viewBox="0 0 24 24"
            fill="currentColor"
